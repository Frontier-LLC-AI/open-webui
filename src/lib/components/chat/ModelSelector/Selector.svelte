...
>
	...
	>
		<div
			class="flex w-full text-left px-0.5 outline-hidden bg-transparent truncate {triggerClassName} justify-between font-medium placeholder-gray-400 focus:outline-hidden text-green-500"
		>
			{#if selectedModel}
				{selectedModel.label}
			{:else}
		...
		{#each filteredItems as item, index}
			...
			>
				<div class="flex flex-col">
					...
					{/if}
					<div class="flex items-center gap-2 {index === selectedModelIdx ? 'text-green-500' : ''}">
						<div class="flex items-center min-w-fit">
							<div class="line-clamp-1">
								<div class="flex items-center min-w-fit">
									<Tooltip
							...
							{#if item.model.owned_by === 'ollama' && (item.model.ollama?.details?.parameter_size ?? '') !== ''}
								<div class="flex ml-1 items-center translate-y-[0.5px]">
									<Tooltip
										...
										className="self-end"
									>
										<span class=" text-xs font-medium text-grey-500 line-clamp-1"
											>{item.model.ollama?.details?.parameter_size ?? ''}</span
										>
									</Tooltip>
								</div>
				...
				{#if value === item.value}
					<div class="text-green-500 ml-auto pl-2 pr-2 md:pr-0">
						<Check />
					</div>
				{/if}
			</button>
